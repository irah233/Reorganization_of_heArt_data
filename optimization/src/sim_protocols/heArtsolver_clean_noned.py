...
def heArtsolver_clean(Tmax_ctrls, Vtargets, Ptargets, mesh, facetboundaries, edgeboundaries, f0, s0, n0, eC0, eL0, eR0, matid, SimDet, IODet, isannotate=True, w_init=None, eCCtargets=None, eLLtargets=None ):
        ...
        fdata = File(outdir+"disp/"+"disp.pvd")
        # Preloading
        ...
        for p in range(0,len(Ptargets)):
            ...
            # save active tension 
            TSpace = TensorFunctionSpace(mesh, "DG", 0)
            Pact = project(Pactive, TSpace)
            file = File(outdir+"Pact.pvd")
            file << Pact
            ...
            if not (eCCtargets is None):
                ...
                for k in range(0, len(eCCtarget_)):
                    j += Kstr*assemble((eCC_ - Constant(eCCtarget_[k]))*dx_(k+1), annotate=isannotate)**2/(assemble(Constant(1.0)*dx_(k+1), annotate=isannotate)**2)

            if not (eLLtargets is None):
                eLLtarget_ = eLLtargets.T[p]
                for k in range(0, len(eLLtarget_)):
                    j += Kstr*assemble((eLL_ - Constant(eLLtarget_[k]))*dx_(k+1), annotate=isannotate)**2/(assemble(Constant(1.0)*dx_(k+1), annotate=isannotate)**2)
...
########################### 
            #print("eCC = ", len(eCC))
            #print("Tmax_arr = ", len(Tmax_arr))
            #print("Tmax_ctrls = ", len(Tmax_ctrls))
            #print("Tmax = ", Tmax)
            #Tmaxarray1 = assemble(Tmax_arr[p]*dx_, annotate=isannotate)

            #file = File(outdir+"Tmax_heart.pvd")
            #file << Tmax
            ...
            #print("Tmax = ", Tmax_arr[p])
            # match Tmax with matid
            Tmaxarray2 = [assemble(Tmax*dx_(q), annotate=isannotate)/assemble(Constant(1.0)*dx_(q), annotate=isannotate) \
                    for q in range(min_matid, max_matid+1)]
            #print("Tmax = ", len(Tmaxarray2))
            Tmaxarray.append(Tmaxarray2)            
###########################
        ...
        return w, j, LVParray, LVVarray, eCCarray, eLLarray, eRRarray, Tmaxarray 

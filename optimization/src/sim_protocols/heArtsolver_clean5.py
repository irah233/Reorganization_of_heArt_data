        ...
        for p in range(0,len(Ptargets)):
            ...
            if not (eCCtargets is None):
                ...
                for k in range(0, len(eCCtarget_)):
                    j += 0 #Kecc*assemble((eCC_ - Constant(eCCtarget_[k]))*dx_(k+1), annotate=isannotate)**2/(assemble(Constant(1.0)*dx_(k+1), annotate=isannotate)**2)

            if not (eLLtargets is None):
                eLLtarget_ = eLLtargets.T[p]
                for k in range(0, len(eLLtarget_)):
                    j += 0 #Kell*assemble((eLL_ - Constant(eLLtarget_[k]))*dx_(k+1), annotate=isannotate)**2/(assemble(Constant(1.0)*dx_(k+1), annotate=isannotate)**2)

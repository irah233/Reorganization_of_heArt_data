...
from ..sim_protocols.heArtsolver_clean5 import heArtsolver_clean
...
def optimization(IODet, SimDet):
    ...
    with open(outdir+'PVLoop.csv','w',newline='') as f:  
        fieldnames = ['LVV','LVP'] 
        n=0
        thewriter = csv.DictWriter(f, fieldnames=fieldnames)
        while n!= (len(LVV_array)-1):
            thewriter.writerow({fieldnames[0]:LVV_array[n],fieldnames[1]:LVP_array[n]})
            n+=1
    ...
    print(len(LVV_array),len(LVP_array))
    ...
    with open(outdir+'Tmax.csv','w',newline='') as f:  
        fieldnames = ['len','Tmax'] 
        n=0
        thewriter = csv.DictWriter(f, fieldnames=fieldnames)
        while n!= (len(Tmax_avg_array)-1):
            thewriter.writerow({fieldnames[0]:n,fieldnames[1]:Tmax_avg_array[n]})
            n+=1
    ...
    for T in f_opt:
        ...
        Tmax_avg, Tmax_std, Tmax_arr = GetTmax(T)
        print("Tmax_arr = ", Tmax_arr)
        print("length of Tmax = ", len(Tmax_arr))
        Tmax_arr_array.append(Tmax_arr)
'''   
    cnt = 0;
    Tmax_arr_array = np.array(Tmax_arr_array)
###############################################################################
    with open(outdir+'HPCC_Ecc.csv','w',newline='') as f:  
        fieldnames = []
        ab=[]
        m=0
###############################################################################
...
###############################################################################           
            ...
            thewriter = csv.DictWriter(f, fieldnames=fieldnames)
            ab.append(eCC_array.T[p].tolist()) 
        while m!=(len(ab[0])-1):                 
            thewriter.writerow({fieldnames[0]:ab[0][m],fieldnames[1]:ab[1][m],fieldnames[2]:ab[2][m],fieldnames[3]:ab[3][m],fieldnames[4]:ab[4][m],fieldnames[5]:ab[5][m],fieldnames[6]:ab[6][m],fieldnames[7]:ab[7][m],fieldnames[8]:ab[8][m],fieldnames[9]:ab[9][m],fieldnames[10]:ab[10][m],fieldnames[11]:ab[11][m],fieldnames[12]:ab[12][m],fieldnames[13]:ab[13][m],fieldnames[14]:ab[14][m],fieldnames[15]:ab[15][m]})
            m+=1
###############################################################################    
        plt.xlabel("Time point")
        plt.ylabel("Ecc")
...
#########################################################
    with open(outdir+'HPCC_Ell.csv','w',newline='') as f:  
        fieldnames = [] 
        ab=[]
        m=0
#########################################################   
...
#########################################################        
            fieldnames.append('Sector'+str(cnt))
            thewriter = csv.DictWriter(f, fieldnames=fieldnames)
            ab.append(eLL_array.T[p].tolist())     
        while m!=(len(ab[0])-1):                 
            thewriter.writerow({fieldnames[0]:ab[0][m],fieldnames[1]:ab[1][m],fieldnames[2]:ab[2][m],fieldnames[3]:ab[3][m],fieldnames[4]:ab[4][m],fieldnames[5]:ab[5][m],fieldnames[6]:ab[6][m],fieldnames[7]:ab[7][m],fieldnames[8]:ab[8][m],fieldnames[9]:ab[9][m],fieldnames[10]:ab[10][m],fieldnames[11]:ab[11][m],fieldnames[12]:ab[12][m],fieldnames[13]:ab[13][m],fieldnames[14]:ab[14][m],fieldnames[15]:ab[15][m]})
            m+=1
#########################################################

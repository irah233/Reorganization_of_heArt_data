...
def GetTmax(Tmax_):
    ...
    try:
        ...
        val_local_sum = 0.0
########
    Tmax_var = Tmax_.vector().get_local()[:]
########
    ...
    return Tmax_avg, Tmax_std, Tmax_var
...
def printTmax(Tmax_array):
    ...
    for m1 in Tmax_array:
        ...
        Tmax_avg, Tmax_std, Tmax_var = GetTmax(m1)
...
def optimization(IODet, SimDet):
    ...
    Tmax_std_array = []
    for T in f_opt:
        Tmax_avg, Tmax_std, Tmax_var = GetTmax(T)
        Tmax_avg_array.append(Tmax_avg)
        Tmax_std_array.append(Tmax_std)
        #print("Tmax_avg = ", Tmax_avg, "Tmax_var = ", Tmax_var)
    ...
    for T in f_opt:
        Tmax_avg, Tmax_std, Tmax_var = GetTmax(T)
        Tmax_var_array.append(Tmax_var)
        for p in range(0, len(Tmax_var)):
            i = int(cnt/4)
            j = cnt%4
            cnt += 1
        #axs[i,j].plot(np.arange(0,len(eLLtargets[p])), eLLtargets[p], '*')
            print("Tmax_var_array[p] = ", Tmax_var_array[p])
            print("Tp = ", Tmax_var[p])
            axs[i,j].plot(np.arange(0,len(Tmax_var_array[p])), Tmax_var_array[p], '-')
            axs[i,j].set_title("Sector = " + str(cnt), fontsize=8)

    plt.xlabel("Time point")
    plt.ylabel("Tmax")
    fig.tight_layout()
    plt.savefig(outdir+"TmaxRegion.png")

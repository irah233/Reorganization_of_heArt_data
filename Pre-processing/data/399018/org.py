...
try:
        ...
        with open("LV_Volume.txt") as f:
            for line in f:
                if line.strip() != '':
                    a1_test4 = [elt.strip() for elt in line.split(',')]
                    if (a1_test4[0][1]) != '.':
                        x_1.append(int(a1_test4[0][0:2])*0.04376)   #time in second
                        y_1.append(a1_test4[0][9:])
                    else:
                        x_1.append(int(a1_test4[0][0])*0.04376)     #time in second
                        y_1.append(a1_test4[0][9:])
        t_1 = x_1
        volume_array_1 =(y_1)
    ...
    with open(txt_filename+'.csv','w',newline='') as f:       #CSV file name
        ...
        while n!= int(data_number):
            thewriter.writerow({'t':t_1[n],'P':P[n],'V':V[n]})  #check length for each list or numpy    -16.14
            n+=1

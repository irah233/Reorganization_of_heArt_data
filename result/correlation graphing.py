...
def twoline(EDV_echo,ESV_echo,EDV_mriw,ESV_mriw,type_):
    ...
    plt.text(80, 60, "R = %.3f" % float(math.sqrt(res_d.rsquared)),fontdict = font1, color = 'blue')
    plt.text(80, 48, "R = %.3f" % float(math.sqrt(res_s.rsquared)),fontdict = font1, color = 'darkorange')
...
unit = '(ms)'
al_bl(mean,diff,md1,md,sd1,sd,'max Tmax time',unit,'max Tmax time')
####################EDV_t
